    ...
    public static setupLanguage() {
      ...
      monaco.languages.registerCompletionItemProvider('myCustomLanguage', {
        provideCompletionItems: function (model, position) {
          ...
          const _suggestions = [];
          if (EditorWidget._words) {
            for (const _word of EditorWidget._words) {
      ...
      if (1 < result.data.length) {
        ...
        if ((0 <= col_tag) && (0 <= col_ali)) {
          for (var i = 1; i < result.data.length; i++) {
            ...
            if (tag) {
              EditorWidget._words.push({
                label: tag,
                insertText: tag.replace('(', '\\(').replace(')', '\\)').replace('_', ' ') + ',',
                documentation: doc,
              });
            }
          }
